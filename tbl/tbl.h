/* tbl.h: external declarations and prototypes */
/* Pure C17 version - converted from C++ class structure */

#ifndef TBL_H
#define TBL_H

#include <stdio.h>
#include <string.h>

/* Constants */
#define MAXLIN 200
#define MAXHEAD 30
#define MAXCOL 20
#define MAXCHS 2000
#define CLLEN 10

/* Column structure */
struct colstr {
    char *col;   /* left column text */
    char *rcol;  /* right column text */
};

/* Global variables - formerly in Parser class */
extern int nlin, ncol, iline, nclin, nslin;
extern int style[MAXHEAD][MAXCOL];
extern int ctop[MAXHEAD][MAXCOL];
extern char font[MAXHEAD][MAXCOL][2];
extern char csize[MAXHEAD][MAXCOL][4];
extern int lefline[MAXHEAD][MAXCOL];
extern char cll[MAXCOL][CLLEN];
extern int stynum[MAXLIN + 1];
extern int F1, F2;
extern struct colstr *table[MAXLIN];
extern int evenup[MAXCOL];
extern int evenflg;
extern int expflg;
extern int ctrflg;
extern int boxflg;
extern int dboxflg;
extern int tab;
extern int allflg;
extern int leftover;
extern int textflg;
extern int left1flg;
extern int rightl;
extern char *cstore;
extern char *cspace;
extern char *last;
extern int sep[MAXCOL];
extern int fullbot[MAXLIN];
extern int instead[MAXLIN];
extern int used[MAXCOL];
extern int lused[MAXCOL];
extern int rused[MAXCOL];
extern int linestop[MAXLIN];
extern const char *ifile;
extern int texname;
extern int texct;
extern char texstr[64];
extern int texmax;
extern int linstart;
extern char *exstore;
extern char *exlim;
extern FILE *tabin;
extern FILE *tabout;

/* Position and line constants */
#define CRIGHT 80
#define CLEFT 40
#define CMID 60
#define S1 31
#define S2 32
#define TMP 38
#define SF 35
#define SL 34
#define LSIZE 33
#define SIND 37

/* Relative position of lines */
#define LEFT 1
#define RIGHT 2
#define THRU 3
#define TOP 1
#define BOT 2

/* Function prototypes */
void tableput(void);
void gettbl(void);
void drawvert(int start, int end, int c, int lwid);
int nodata(int il);
int oneh(int lin);
void permute(void);
int vspand(int ir, int ij);
int vspen(char *s);
void maktab(void);
void wide(char *s, char *fn, char *size);
void runout(void);
void runtabs(int i);
int ifline(char *s);
void need(void);
void deftail(void);
void putline(int i, int nl);
void puttext(char *s, char *fn, char *size);
void funnies(int stl, int lin);
void putfont(char *fn);
void putsize(char *s);
void yetmore(void);
void checkuse(void);
int real(char *s);
char *chspace(void);
struct colstr *alocv(int n);
void release(void);
void choochar(void);
int point(char *s);
void error(char *s);
char *gets1(char *s);
void un1getc(int c);
int get1char(void);
void savefill(void);
void rstofill(void);
void endoff(void);
void ifdivert(void);
void saveline(void);
void restline(void);
void cleanfc(void);
int gettext(char *sp, int ilin, int icol, char *fn, char *sz);
void untext(void);
int interv(int i, int c);
int interh(int i, int c);
int up1(int i);
char *maknew(char *str);
int match(char *s1, char *s2);
int prefix(char *small, char *big);
int letter(int ch);
int numb(char *str);
int digit(int x);
int max(int a, int b);
int index(char *s, int c);
void tcopy(char *s, char *t);

/* from tt.c */
int ctype(int il, int ic);
int min(int a, int b);
int fspan(int i, int c);
int lspan(int i, int c);
int ctspan(int i, int c);
void tohcol(int ic);
int allh(int i);
int thish(int i, int c);

/* from tu.c */
void makeline(int i, int c, int lintype);
void fullwide(int i, int lintype);
void drawline(int i, int cl, int cr, int lintype, int noheight);
void getstop(void);
int left(int i, int c, int *lwidp);
int lefdata(int i, int c);
int next(int i);
int prev(int i);

#endif /* TBL_H */
