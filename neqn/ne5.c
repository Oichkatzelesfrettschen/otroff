...
void mark(int n) 
{
    ...
    if (n != 0 && !validate_object_handle(n)) {
        if (dbg) printf(".\tERROR: mark: invalid object handle n=%d\n", n);
        return;
    }
    ...
    if (n != 0) {
        ...
        if (!validate_object_handle(yyval)) {
            if (dbg) printf(".\tERROR: mark: failed to allocate marker handle\n");
            return;
        }
...
void lineup(int n) 
{
    /* Validate input handle if non-zero */
    if (n != 0 && !validate_object_handle(n)) {
        if (dbg) printf(".\tERROR: lineup: invalid object handle n=%d\n", n);
        return;
    }
    ...
    if (n != 0) {
        ...
        if (!validate_object_handle(yyval)) {
            if (dbg) printf(".\tERROR: lineup: failed to allocate cursor handle\n");
            return;
        }
