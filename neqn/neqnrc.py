...
class NeqnBuilder:
    ...
    def check_dependencies(self) -> bool:
            ...
            try:
                with open(grammar_path, "r", encoding="utf-8") as f:
                    ...
                    if not content.strip():
                        self.logger.warning(
                            f"Grammar file {self.grammar_file} appears to be empty"
                        )
                    elif not any(
                        keyword in content for keyword in ["%token", "%type", "%%", ":"]
                    ):
                        self.logger.warning(
                            f"Grammar file {self.grammar_file} may not be valid yacc grammar"
                        )
            except (UnicodeDecodeError, IOError) as e:
                self.logger.warning(
                    f"Could not validate grammar file {self.grammar_file}: {e}"
                )
        ...
        if not source_files:
            missing_items.append(
                f"Source files matching pattern: {self.source_pattern}"
            )
        else:
                ...
                for source_file in source_files:
                    try:
                        with open(source_file, "r", encoding="utf-8") as f:
                            ...
                            if not content.strip():
                                self.logger.warning(
                                    f"Source file {source_file.name} appears to be empty"
                                )
                            elif not any(
                                keyword in content
                                for keyword in ["#include", "int ", "char ", "void "]
                            ):
                                self.logger.warning(
                                    f"Source file {source_file.name} may not be valid C code"
                                )
                    except (UnicodeDecodeError, IOError) as e:
                        self.logger.warning(
                            f"Could not validate source file {source_file.name}: {e}"
                        )
    ...
    def generate_parser(self) -> bool:
        ...
        success, output = self.run_command(
            yacc_command, "Generating parser from grammar"
        )
        ...
        if success:
            ...
            for generated_file in self.generated_files:
                ...
                if file_path.exists():
                    ...
                    self.logger.debug(
                        f"Generated file: {generated_file} ({file_size} bytes)"
                    )
                else:
                    self.logger.warning(
                        f"Expected generated file not found: {generated_file}"
                    )
    ...
    def compile_sources(self) -> bool:
        ...
        if not source_files:
            self.logger.error(
                f"No source files found matching pattern: {self.source_pattern}"
            )
            return False
        ...
        # Execute compilation
        success, output = self.run_command(
            compile_command, f"Compiling {len(source_files)} source files"
        )
    ...
    def rename_executable(self) -> bool:
        ...
        if not temp_executable:
            self.logger.error(
                "No compiled executable found (looked for: {})".format(
    ...
    def clean_build(self) -> bool:
        ...
        if files_removed:
            self.logger.info(
                f"Removed {len(files_removed)} artifacts: {', '.join(files_removed)}"
            )
        else:
...
Examples:
  ...
  python3 neqnrc --no-cleanup       # Skip cleaning intermediate files
    ...
    builder = NeqnBuilder(build_dir=args.build_dir, verbose=args.verbose)
