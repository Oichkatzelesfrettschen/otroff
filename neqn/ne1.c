...
void boverb(int p1, int p2) 
{
    ...
    if (!validate_object_handle(p1) || !validate_object_handle(p2)) {
        if (dbg) printf(".\tERROR: boverb: invalid handles p1=%d, p2=%d\n", p1, p2);
        return;
    }

    treg = oalloc(); /* Allocate a temporary register for width calculations */
    if (!validate_object_handle(treg)) {
        if (dbg) printf(".\tERROR: boverb: failed to allocate temporary register\n");
        return;
    }
...
void bshiftb(int p1, int dir, int p2) 
{
    ...
    /* Validate input parameters */
    if (!validate_object_handle(p1) || !validate_object_handle(p2)) {
        if (dbg) printf(".\tERROR: bshiftb: invalid handles p1=%d, p2=%d\n", p1, p2);
        return;
    }
...
void eqnbox(int p1, int p2) 
{
    ...
    /* Validate input parameters */
    if (!validate_object_handle(p1) || !validate_object_handle(p2)) {
        if (dbg) printf(".\tERROR: eqnbox: invalid handles p1=%d, p2=%d\n", p1, p2);
        return;
    }
...
void size(int p1, int p2) 
{
    ...
    if (p2 < 4 || p2 > 72) {
        if (dbg) printf(".\tWARNING: size: unusual size value %d\n", p2);
    }
...
int numb(char *p1) 
{
    ...
    /* Input validation */
    if (p1 == NULL) {
        if (dbg) printf(".\tERROR: numb: null string pointer\n");
        return 0;
    }
...
void shift(int p1) 
{
    ...
    if (!validate_object_handle(p1)) {
        if (dbg) printf(".\tERROR: shift: invalid handle p1=%d\n", p1);
        return;
    }
...
void sqrt_neqn(int p2) 
{
    ...
    if (!validate_object_handle(p2)) {
        if (dbg) printf(".\tERROR: sqrt_neqn: invalid handle p2=%d\n", p2);
        return;
    }
...
void lpile(int type, int p1, int p2) 
{
    ...
    if (p1 < 0 || p2 < p1 || p2 - p1 > 100) {
        if (dbg) printf(".\tERROR: lpile: invalid range p1=%d, p2=%d\n", p1, p2);
        return;
    }

    yyval = oalloc(); /* Allocate new handle for pile object */
    if (!validate_object_handle(yyval)) {
        if (dbg) printf(".\tERROR: lpile: failed to allocate pile handle\n");
        return;
    }
    ...
    nlist = p2 - p1;
    if (nlist <= 0) {
        if (dbg) printf(".\tDEBUG: lpile: empty pile range %d to %d\n", p1, p2);
        eht[yyval] = 0;
        ebase[yyval] = 0;
        printf(".ds %d \"\"\n", yyval);
        return;
    ...
    for (i = p1; i < p2; i++) {
        if (!validate_object_handle(lp[i])) {
            if (dbg) printf(".\tERROR: lpile: invalid element handle lp[%d]=%d\n", i, lp[i]);
            continue;
        }
        h_sum += eht[lp[i]];
    }
    ...
    for (i = p2 - 1; i >= p1; i--) {
        if (!validate_object_handle(lp[i])) continue;
        ...
        default:
            if (dbg) printf(".\tWARNING: lpile: unknown type '%c'\n", type);
            printf("\\*(%d", lp[i]);
            break;
        }
...
void shift2(int p1, int p2, int p3) 
{
    ...
    if (!validate_object_handle(p1) || !validate_object_handle(p2) || 
        ...
        if (dbg) printf(".\tERROR: shift2: invalid handles p1=%d, p2=%d, p3=%d\n", 
                        p1, p2, p3);
        return;
    }
    ...
    treg = oalloc();
    if (!validate_object_handle(treg)) {
        if (dbg) printf(".\tERROR: shift2: failed to allocate temporary register\n");
        return;
    }
