...
void fromto(int p1, int p2, int p3) 
{
    ...
    if (!validate_object_handle(p1)) {
        if (dbg) printf(".\tERROR: fromto: invalid main handle p1=%d\n", p1);
        return;
    }

    /* Validate optional components if present */
    if (p2 > 0 && !validate_object_handle(p2)) {
        if (dbg) printf(".\tERROR: fromto: invalid from handle p2=%d\n", p2);
        return;
    }
    if (p3 > 0 && !validate_object_handle(p3)) {
        if (dbg) printf(".\tERROR: fromto: invalid to handle p3=%d\n", p3);
        return;
    }

    yyval = oalloc(); /* Allocate new handle for combined expression */
    if (!validate_object_handle(yyval)) {
        if (dbg) printf(".\tERROR: fromto: failed to allocate result handle\n");
        return;
    }
...
void paren(int leftc, int p1, int rightc) 
{
    ...
    if (!validate_object_handle(p1)) {
        if (dbg) printf(".\tERROR: paren: invalid expression handle p1=%d\n", p1);
        return;
    }
    ...
    if (n < 2) n = 2; /* Ensure minimum height */
    ...
    if (leftc == '{' || rightc == '}') {
        ...
        if (n < 3) n = 3; /* Minimum height for proper brace */
    ...
    case '{':   /* left brace - multi-part construction */
        ...
        for (j = 0; j < m; j += 2) printf("\\(bv"); /* extension pieces */
        printf("\\(lk"); /* middle piece */
        for (j = 0; j < m; j += 2) printf("\\(bv"); /* more extensions */
...
void brack(int m, char *t, char *c, char *b) 
{
    ...
    /* Input validation */
    if (!t || !c || !b) {
        if (dbg) printf(".\tERROR: brack: null pointer in parameters\n");
        return;
    }

    if (m < 0) {
        if (dbg) printf(".\tWARNING: brack: negative extension count %d\n", m);
        m = 0;
    }
...
void diacrit(int p1, int type) 
{
    ...
    /* Validate expression parameter */
    if (!validate_object_handle(p1)) {
        if (dbg) printf(".\tERROR: diacrit: invalid expression handle p1=%d\n", p1);
        return;
    }
    ...
    if (!validate_object_handle(c) || !validate_object_handle(t)) {
        if (dbg) printf(".\tERROR: diacrit: failed to allocate handles\n");
        return;
    }
    ...
    default:
        if (dbg) printf(".\tWARNING: diacrit: unknown type '%c'\n", type);
...
void move(int dir, int amt, int p) 
{
    ...
    if (!validate_object_handle(p)) {
        if (dbg) printf(".\tERROR: move: invalid expression handle p=%d\n", p);
        return;
    }

    /* Validate direction parameter */
    if (dir < 0 || dir > 3) {
        if (dbg) printf(".\tERROR: move: invalid direction %d\n", dir);
        return;
    }
...
void funny(int n) 
{
    ...
    if (!validate_object_handle(yyval)) {
        if (dbg) printf(".\tERROR: funny: failed to allocate symbol handle\n");
        return;
    }
    ...
    default:
        if (dbg) printf(".\tWARNING: funny: unknown symbol code '%c'\n", n);
