...
static int validate_matrix_parameters(int p1, int p2) 
{
    ...
    if (p1 < 0 || p1 >= MAX_MATRIX_ELEMENTS) {
        if (dbg) printf(".\tERROR: validate_matrix: p1=%d out of bounds\n", p1);
        return -1;
    }
    
    if (ct < 0 || ct >= MAX_MATRIX_ELEMENTS) {
        if (dbg) printf(".\tERROR: validate_matrix: ct=%d out of bounds\n", ct);
        return -1;
    }
    
    if (p1 >= ct) {
        if (dbg) printf(".\tERROR: validate_matrix: p1=%d >= ct=%d\n", p1, ct);
        return -1;
    }
    ...
    *b = 0;
    ...
    for (i = 0; i < ncol; i++) {
        ...
        if (!validate_layout_bounds(j)) {
            if (dbg) printf(".\tERROR: calculate_row_dimensions: invalid index %d\n", j);
            return;
        }
...
static void apply_row_dimensions(int row, int p1, int ncol, int nrow, int hb, int b) 
{
    ...
    for (i = 0; i < ncol; i++) {
        /* Validate array bounds before access */
        if (!validate_layout_bounds(j)) {
            if (dbg) printf(".\tERROR: apply_row_dimensions: invalid index %d\n", j);
            return;
        }
...
void column(int type, int p1, int p2) 
{
    ...
    if (ct >= MAX_MATRIX_ELEMENTS - 1) {
        if (dbg) printf(".\tERROR: column: layout array full, ct=%d\n", ct);
        return;
    }
...
void matrix(int p1, int p2) 
{
    ...
    if (nrow <= 0 || nrow > MAX_MATRIX_ROWS) {
        if (dbg) printf(".\tERROR: matrix: invalid row count %d\n", nrow);
        return;
    }
    ...
    while (i < ct && ncol < MAX_MATRIX_COLUMNS) {
        /* Validate bounds before accessing lp[i] */
        if (!validate_layout_bounds(i)) {
            if (dbg) printf(".\tERROR: matrix: invalid layout index %d\n", i);
            return;
        }
    ...
    if (ncol == 0) {
        if (dbg) printf(".\tERROR: matrix: no columns found\n");
        return;
    }
    ...
    for (i = 0; i < ncol; i++) {
        ...
        if (!validate_layout_bounds(j) || !validate_layout_bounds(j + lp[j] + 1)) {
            if (dbg) printf(".\tERROR: matrix: invalid pile bounds for column %d\n", i);
            return;
        }
    ...
    yyval = oalloc();
    if (yyval <= 0) {
        if (dbg) printf(".\tERROR: matrix: failed to allocate result handle\n");
        return;
    }
