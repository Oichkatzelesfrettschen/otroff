...
function(detect_build_environment_minimal)
    ...
    if(NOT BUILD_ENV_HAS_CONCEPTS_FLAG_UNAWARE AND (BUILD_ENV_CXX_STANDARD STREQUAL "20" OR BUILD_ENV_CXX_STANDARD STREQUAL "23"))
        ...
        set(CMAKE_REQUIRED_FLAGS "-std=c++${BUILD_ENV_CXX_STANDARD}")
        check_cxx_source_compiles("${CXX_CONCEPTS_CODE}" BUILD_ENV_HAS_CONCEPTS)
    ...
    if(NOT BUILD_ENV_HAS_COROUTINES_FLAG_UNAWARE AND (BUILD_ENV_CXX_STANDARD STREQUAL "20" OR BUILD_ENV_CXX_STANDARD STREQUAL "23"))
        ...
        set(CMAKE_REQUIRED_FLAGS "-std=c++${BUILD_ENV_CXX_STANDARD}")
        check_cxx_source_compiles("${CXX_COROUTINES_CODE}" BUILD_ENV_HAS_COROUTINES)
        unset(CMAKE_REQUIRED_FLAGS)
    else()
    ...
    if(BUILD_ENV_COMPILER_IS_GCC)
        ...
        if(GCC_HAS_MARCH_NATIVE)
            message(STATUS "GCC: -march=native supported")
        else()
            message(STATUS "GCC: -march=native not supported")
        endif()
        ...
        if(GCC_HAS_LTO)
            message(STATUS "GCC: -flto supported")
        else()
            message(STATUS "GCC: -flto not supported")
        endif()
        ...
        if(GCC_HAS_COLOR_DIAG)
            message(STATUS "GCC: -fdiagnostics-color=always supported")
        else()
            message(STATUS "GCC: -fdiagnostics-color=always not supported")
        endif()
        ...
        if(CMAKE_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "10.0")
            ...
            if(GCC_HAS_CONCEPTS_DIAG_DEPTH)
                message(STATUS "GCC: -fconcepts-diagnostics-depth=3 supported")
            else()
                message(STATUS "GCC: -fconcepts-diagnostics-depth=3 not supported")
            endif()
        else()
