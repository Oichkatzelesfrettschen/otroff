 ...
 *
 * Layout:
 *   - suftab_index: offsets into the suffix byte table for each initial
 *     letter. Each value is a byte offset.
 *   - suftab_bytes: encoded suffix entries. Each entry begins with a length
 *     byte and may use the high bits as flags for the hyphenation routine.
 */
#include <stddef.h>

const unsigned short suftab_index[26] = {
    0x0044, 0x0000, 0x006d, 0x00c9, 0x017b, 0x0334, 0x0345, 0x03d3,
    0x043b, 0x0000, 0x044d, 0x0467, 0x04f8, 0x0537, 0x05eb, 0x05f1,
    0x0000, 0x0601, 0x06f2, 0x073a, 0x0000, 0x0000, 0x0000, 0x0000,
    0x07cd, 0x0000,
};
...
static const unsigned char suftab_bytes[] = {
    ...
    0x65, 0x65, 0x64, 0x02, 0xe2, 0x03, 0xf7, 0x61, 0x03, 0xe4, 0x61, 0x00,
};

unsigned short suftab_get_offset(size_t idx)
{
    return idx < 26 ? suftab_index[idx] : 0;
}

unsigned char suftab_get_byte(size_t idx)
{
    return idx < sizeof(suftab_bytes) ? suftab_bytes[idx] : 0;
}
